...
export function HomeScreen({ result, onStart, onOpenNote }) {
  return (
    <div className="space-y-5 pb-6">
      ...
      <div className="px-5">
        <section className="rounded-[32px] bg-slate-950 p-5 text-white shadow-[0_20px_48px_rgba(15,23,42,0.22)]">
          ...
          <p className="mt-3 text-[14px] leading-6 text-slate-300">
            当前前端已改为围绕 AgentResult 渲染，后续可以直接接入后端 API。
          </p>
      ...
      <div className="px-5">
        <Card title="最近演示结果" subtitle="Demo Case">
          <button onClick={onOpenNote} className="w-full rounded-3xl border border-slate-200 bg-slate-50 p-4 text-left">
            <div className="flex items-start justify-between gap-3">
              <div>
                <h3 className="text-[17px] font-semibold text-slate-900">{result.topic}</h3>
                <p className="mt-2 text-[13px] leading-5 text-slate-500">{result.summary}</p>
              </div>
              <span className="rounded-full bg-blue-50 px-3 py-1 text-[12px] font-semibold text-blue-600">可追溯</span>
            </div>
          </button>
        </Card>
      </div>
